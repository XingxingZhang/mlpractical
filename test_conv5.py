...
from mlp.layers import ReluLayer, AffineLayer, DropoutLayer, ReshapeLayer, ConvolutionalLayer, MaxPooling2DLayer
...
def test_conv(params):
    ...
    model = MultipleLayerModel([
        DropoutLayer(rng, inDropout),
        ReshapeLayer((1, 28, 28)),
        ConvolutionalLayer(1, 6, 28, 28, 5, 5),
        ReluLayer(),
        MaxPooling2DLayer((2, 2)),
        ConvolutionalLayer(6, 16, 12, 12, 5, 5),
        ReluLayer(),
        MaxPooling2DLayer((2, 2)),
        ReshapeLayer(),
        AffineLayer(256, 200, weights_init, biases_init),
        ReluLayer(),
        DropoutLayer(rng, dropout),
        AffineLayer(200, 100, weights_init, biases_init),
        ReluLayer(),
        DropoutLayer(rng, dropout),
        AffineLayer(100, 10, weights_init, biases_init)
    ])
    ...
    learning_rule = MomentumLearningRule(0.02, 0.95)
